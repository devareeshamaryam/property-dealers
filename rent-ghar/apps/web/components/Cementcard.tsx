...
function CementImage({ item, svgWidth = "120px", svgHeight = "150px" }: { item: CementBrand; svgWidth?: string; svgHeight?: string }) {
...
export default function CementCard({ item, viewMode = "grid" }: CementCardProps) {
        ...
        return (
            ...
            >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: "12px", color: "#6b7280", marginTop: "2px" }}>{item.city} · {item.weightKg} Kg</p>
                </div>
    ...
    return (
        ...
        >
            ...
            <style>{`
        .cement-card .cc-top   { padding: 14px 14px 12px; min-height: 74px; }
        .cement-card .cc-cat   { font-size: 11px; color: #9ca3af; margin-bottom: 5px; line-height: 1; }
        .cement-card .cc-title { font-size: 13px; font-weight: 400; color: #1a6fc4; text-decoration: none; line-height: 1.45; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
        .cement-card .cc-img   { display: flex; align-items: center; justify-content: center; background: #fff; padding: 12px 40px; height: 210px; }
        .cement-card .cc-bot   { display: flex; align-items: center; justify-content: space-between; padding: 12px 14px; border-top: 1px solid #f3f4f6; }
        .cement-card .cc-price { font-size: 18px; font-weight: 600; color: #1f2937; }
        .cement-card .cc-btn   { width: 36px; height: 36px; }

        @media (max-width: 639px) {
          .cement-card .cc-top   { padding: 7px 7px 5px; min-height: 50px; }
          .cement-card .cc-cat   { font-size: 9px; margin-bottom: 2px; }
          .cement-card .cc-title { font-size: 10px; }
          .cement-card .cc-img   { padding: 6px 12px; height: 110px; }
          .cement-card .cc-bot   { padding: 6px 8px; }
          .cement-card .cc-price { font-size: 12px; }
          .cement-card .cc-btn   { width: 26px; height: 26px; }
        }
      `}</style>
            ...
            <Link href={`/today-cement-rate-in-pakistan/${item.slug}`} className="cc-img">
                <CementImage item={item} svgWidth="120px" svgHeight="150px" />
            </Link>
...
function CartButton({ inCart, onClick }: { inCart: boolean; onClick: (e: React.MouseEvent) => void }) {
    return (
        <button
            onClick={onClick}
            ...
            style={{
                borderRadius: "50%",
                border: `1px solid ${inCart ? "#22c55e" : "#e5e7eb"}`,
                background: inCart ? "#22c55e" : "#f3f4f6",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                cursor: "pointer",
                color: inCart ? "#fff" : "#6b7280",
        ...
        >
            {inCart ? <CheckCircle2 width={13} height={13} /> : <ShoppingCart width={13} height={13} />}
        </button>
    );
}