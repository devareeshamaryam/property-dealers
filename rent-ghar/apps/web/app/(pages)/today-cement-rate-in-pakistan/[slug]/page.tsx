...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    return {
      ...
      description: `Today's ${rate.brand} cement price in ${rate.city} is Rs ${rate.price}. Check latest updates and order online.`,
      openGraph: {
        title: `${rate.brand} Cement Price Today`,
        description: `Rs ${rate.price} — ${rate.city}`,
        images: rate.image
...
export default async function CementDetailPage({ params }: Props) {
  ...
  return <CementDetailClient rate={rate} />;
}