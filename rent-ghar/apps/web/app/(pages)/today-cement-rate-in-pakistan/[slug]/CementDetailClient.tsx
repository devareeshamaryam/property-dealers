...
import {
  ShoppingCart, CheckCircle2, TrendingUp, TrendingDown,
  Minus, Package, MapPin, ChevronLeft, ChevronRight,
  Shield, Truck, Factory, Clock, Facebook, Mail, Linkedin, Phone,
} from 'lucide-react';
...
export default function CementDetailClient({ rate }: { rate: CementRate }) {
  ...
  const inCart  = isInCart(rate._id);
  ...
  const handleAddToCart = () => {
    for (let i = 0; i < qty; i++) {
      addItem({
        ...
        slug: rate.slug,
        price: rate.price,
        weightKg: rate.weightKg ?? 50,
        category: rate.category ?? 'OPC Cement',
        city: rate.city,
        image: rate.image,
      });
    }
    toast.success(`${rate.brand} added to cart!`);
  };
      ...
      <div className="max-w-6xl mx-auto px-3 py-5">
        ...
        <div className="grid lg:grid-cols-2 gap-5">
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden bg-gray-50">
            <div className="relative w-full h-[360px] bg-gray-50">
              {images.length > 0 ? (
                <>
                  <img
                    key={activeImg}
              ...
              <button
                onClick={prevImg}
                className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded bg-white border border-gray-300 shadow flex items-center justify-center hover:bg-gray-100 transition-colors z-10"
              >
                <ChevronLeft className="w-4 h-4 text-gray-600" />
              </button>
              <button
                onClick={nextImg}
                className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded bg-white border border-gray-300 shadow flex items-center justify-center hover:bg-gray-100 transition-colors z-10"
              >
              ...
              <div className="flex gap-1.5 p-2 border-t border-gray-100 overflow-x-auto">
                {images.map((img, i) => (
                  <button
                    key={i}
                    onClick={() => setActiveImg(i)}
                    className={`flex-shrink-0 w-12 h-12 rounded overflow-hidden border transition-all ${i === activeImg ? 'border-black' : 'border-gray-200 hover:border-gray-400'}`}
                  >
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                { icon: MapPin,  label: 'City',     value: rate.city },
            ...
            {/* ── CART: Qty counter + Add to Cart button (commented out — uncomment to re-enable)
            <div className="flex items-center gap-2">
              <div className="flex items-center border border-gray-300 rounded overflow-hidden">
                <button onClick={() => setQty(q => Math.max(1, q - 1))} className="w-8 h-8 flex items-center justify-center bg-gray-100 hover:bg-gray-200 font-bold text-sm transition-colors">−</button>
                <span className="w-9 text-center text-sm font-semibold">{qty}</span>
                <button onClick={() => setQty(q => q + 1)} className="w-8 h-8 flex items-center justify-center bg-gray-100 hover:bg-gray-200 font-bold text-sm transition-colors">+</button>
              </div>
              <button
                onClick={handleAddToCart}
                className={`flex-1 h-8 rounded flex items-center justify-center gap-1.5 text-sm font-semibold transition-all ${
                  inCart ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : 'bg-black hover:bg-gray-800 text-white'
                }`}
              >
                {inCart ? <><CheckCircle2 className="w-3.5 h-3.5" /> In Cart</> : <><ShoppingCart className="w-3.5 h-3.5" /> Add to Cart</>}
              </button>
            </div>
            ── END CART ── */}
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <a
                href={`https://wa.me/${CONTACT_NUMBER}?text=${encodeURIComponent(`Hi, I'm interested in ${rate.brand} — Rs ${rate.price}`)}`}
                target="_blank"
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {[
                ...
                { icon: Factory, label: 'Direct from Factory' },
