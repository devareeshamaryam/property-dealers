...
export default async function SaleRootPage({ searchParams }: PageProps) {
  ...
  if (cityName) {
    try {
      ...
      cityRichDescription = specificContent?.content || cityDetails?.saleContent || '';
    } catch (e) {
  ...
  try {
    const q = cityName ? `city=${cityName}&purpose=sale&limit=20` : 'purpose=sale&limit=20';
  ...
  const collectionSchema = buildCollectionPageSchema({
    ...
    crumbs: [
      ...
      ...(cityName ? [{ name: toTitleCase(cityName), url: `${BASE_URL}/properties/sale/${cityName.toLowerCase()}` }] : [])
    ]
  });
